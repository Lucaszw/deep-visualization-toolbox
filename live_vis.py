...
class LiveVis(object):
    ...
    def __init__(self, settings=None, onChange=None):
        # self.init_sockets()
        print("SETTINGS:");
        print(settings);
        ...
        while not self.quit:
            ...
            if imshow_needed:
                ...
                with WithTimer('LiveVis:imshow', quiet = self.debug_level < 1):
                    print("SETTINTS_TIMER:")
                    print(self.settings.caffevis_caffe_root)
                    ...
                    if self.help_mode:
                        ...
                        img_data = self.help_buffer[:,:,::-1]
                    else:
                        # cv2_imshow_rgb(self.window_name, self.window_buffer)
                        img_data = self.window_buffer[:,:,::-1]

                    cnt = cv2.imencode('.png',img_data)[1]
                    b64 = base64.encodestring(cnt)

                    if not self.quit:
                        self.onChange(data=b64)
    ...
    def handle_key_pre_apps(self, key=None,tag=None):
        if tag == None:
